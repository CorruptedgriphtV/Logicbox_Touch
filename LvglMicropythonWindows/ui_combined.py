...
def create_editor_screen():
    ...
    code_display_ta.set_size(300, 50)
    code_display_ta.align(lv.ALIGN.IN_TOP_MID, 0, 100)
    code_display_ta.set_placeholder_text("Visualización del código...")
    code_display_ta.set_editable(False)
    ...
    def save_code(e):
        ...
        try:
            with open("user_code.py", "w") as f:
                f.write(code)
            code_display_ta.set_text("Código guardado en user_code.py")
        except Exception as ex:
            code_display_ta.set_text("Error al guardar el código: " + str(ex))

    # Botón para guardar el código
    save_btn = lv.btn(scr)
    save_btn.set_size(100, 30)
    save_btn.align(lv.ALIGN.TOP_LEFT, 10, 10)
    save_btn.add_event_cb(save_code, lv.EVENT.CLICKED, None)
    save_label = lv.label(save_btn)
    save_label.set_text("Guardar")
    save_label.center()

    # Botón para regresar a la pantalla principal
    btn_back = lv.btn(scr)
    btn_back.set_size(100, 30)
    btn_back.align(lv.ALIGN.BOTTOM_LEFT, 10, -10)
    btn_back.add_event_cb(lambda _: lv.scr_load(main_scr), lv.EVENT.CLICKED, None)
    label_back = lv.label(btn_back)
    label_back.set_text("Regresar")
    label_back.center()
